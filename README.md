...
```bash
music-converter$ sam local start-api
music-converter$ curl http://localhost:3000/
```
...
```bash
music-converter$ pip install -r tests/requirements.txt --user
# unit test
music-converter$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
music-converter$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name music-converter
```
